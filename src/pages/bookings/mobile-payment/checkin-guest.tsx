// - - - src/pages/bookings/mobile-payment/checkin-guest.tsx
...
export const CheckinGuest: React.FC<CheckinGuestProps> = ({ bookingId }) => {
  ...
  const handleCheckIn = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const errorMessage = errorData.detail || "Check-in failed.";
  ...
  if (isLoading) {
    return (
      <Card className="w-[540px] max-w-[640px] border-gray-200 shadow-sm text-center">
        <CardContent className="p-6">
          ...
          <CardDescription className="text-gray-600">
            This may take a minute or two. Please do not refresh the page.
          </CardDescription>
        </CardContent>
      </Card>
    );
  ...
  if (error) {
    return (
      <Card className="max-w-md mx-auto border-gray-200 shadow-sm text-center">
        <CardContent className="p-6">
  ...
  if (finalBooking?.payment_status !== "Paid") {
    return (
      <Card className="max-w-md mx-auto border-gray-200 shadow-sm text-center">
        <CardContent className="p-6">
          ...
          <CardDescription className="text-yellow-700">
            ...
            Status: <strong>{finalBooking?.payment_status}</strong>
          </CardDescription>
          <p className="text-sm text-gray-600 mt-2">
            Please check your payment app to complete the transaction or contact
            support.
          </p>
        </CardContent>
      </Card>
    ...
    <div>
      <div className="flex justify-between items-center mb-6 non-printable">
        ...
        <div className="flex gap-3 mx-4">
          <Button
            onClick={handlePrint}
            className="bg-gray-600 rounded-md shadow hover:bg-gray-700 text-white"
          >
          ...
          <Button
            ...
            className="bg-blue-600 rounded-md shadow hover:bg-blue-700 disabled:bg-gray-400 text-white"
          >
            {isCheckingIn ? (
      ...
      >
        <CardHeader className="bg-gray-50/10 shadow border-b">
        ...
        <CardContent className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              <p className="flex items-center text-gray-600">
                ...
                <span className="ml-1">
                  {finalBooking.booking_status} / {finalBooking.payment_status}
                </span>
              </p>
            </div>
          </div>
          <div className="mt-6">
            ...
            <div className="mt-4 space-y-2">
              {finalBooking.billing_meta_data.charge_report.map((item) => (
                <div
                  key={item.item_type}
              ...
              <div className="flex justify-between font-bold text-lg text-gray-800">
                <span>Total Paid (TZS)</span>
                <span>
                  {parseFloat(finalBooking.amount_paid).toLocaleString(
                    "en-US",
                    {
                      style: "currency",
                      currency: "TZS",
