...
export default function HotelDepartments() {
  ...
  const {
    ...
    queryKey: ["departments", hotelId], // ✨ Step 6: Update queryKey
    queryFn: async () => {
      const params = new URLSearchParams({ hotel_id: hotelId! }); // ✨ Step 5: Update API parameter
      const response = await apiClient.get(`departments/`, { params });
      return response.data;
    },
  ...
  const createDepartmentMutation = useMutation({
    mutationFn: (newDepartment: DepartmentFormValues) => {
      const payload = { ...newDepartment, hotel: hotelId! }; // ✨ Step 5: Update API parameter
      return apiClient.post("departments/", payload);
    },
    onSuccess: () => {
      toast.success("Department created successfully!");
