...
// --- ✨ NEW: Helper function for progress bar color ---
const getProgressIndicatorClass = (percentage: number): string => {
  if (percentage < 40) return "bg-orange-500";
  if (percentage < 99) return "bg-blue-500";
  return "bg-green-500";
};
...
export const Step6_Finish = () => {
  ...
  const progressValue = progress?.progress_percentage ?? 0; // --- Get value
    ...
    <div className="flex flex-col items-center justify-center text-center p-8 space-y-8 min-h-[400px]">
      <div className="w-full max-w-3xl mx-auto">
        ...
        <div className="text-left space-y-2 mb-8">
          ...
          {/* --- ✨ UPDATE: Apply the dynamic class to the progress bar --- */}
          <Progress
            value={progressValue}
            className="w-full h-3"
            indicatorClassName={getProgressIndicatorClass(progressValue)}
          />
        </div>
