...
//   // --- UPDATED: Get the 'error' state from the store ---
...
//       await login(data);
//     } catch (error) {
//       // The store now holds the error state, so no extra action is needed here.
...
//                   Welcome Back
//                 </h2>
//                 {/* --- UPDATED: Conditional error message --- */}
//                 {error ? (
...
// GROK src/pages/authentication/login.tsx
// import { useState, useEffect, type ReactNode } from "react";
// import { useForm } from "react-hook-form";
// import { zodResolver } from "@hookform/resolvers/zod";
// import * as z from "zod";
...
//   // --- State and Hooks ---
//   const navigate = useNavigate();
//   const { login, isLoading, isAuthenticated, error, isFirstLogin } =
//     useAuthStore();
//   const [showPassword, setShowPassword] = useState(false);
//   const [minPanelSize, setMinPanelSize] = useState(25);
...
//       } else {
//         navigate("/", { replace: true }); // Covers isFirstLogin === false or null
//       }
//     }
...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (isAuthenticated) {
      // isFirstLogin will be true, false, or null. We only act when it's not null.
      if (isFirstLogin === true) {
        navigate("/welcome", { replace: true });
  ...
  }, [isAuthenticated, isFirstLogin, navigate]); // Add isFirstLogin to dependencies
